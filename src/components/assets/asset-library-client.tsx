...
type AssetType = "Logo" | "Flyer" | "Video" | "Photo" | "Document" | "B-roll" | "Reference" | "Other";
type AssetStatus = "Raw" | "In Progress" | "Ready" | "Approved" | "Used" | "Archived";
...
const assetTypes: AssetType[] = ["Logo", "Flyer", "Video", "Photo", "Document", "B-roll", "Reference", "Other"];
const statuses: AssetStatus[] = ["Raw", "In Progress", "Ready", "Approved", "Used", "Archived"];
...
const starterAssets: AssetRecord[] = [
  {
    ...
    notes: "Use only the accurate official logo. Do not recreate or fake this asset.",
    createdAt: new Date().toISOString(),
  },
  {
    id: "starter-documentary-frames",
    ...
    priority: "High",
    link: "",
    tags: "documentary, tax, frames, visuals",
    notes: "Premium 4K government documentary visuals and scene frames.",
    createdAt: new Date().toISOString(),
  },
];
...
function iconForType(type: AssetType) {
  ...
  if (type === "Flyer" || type === "Photo" || type === "Logo") return ImageIcon;
  return FileText;
}

function badgeClass(value: string) {
  if (value === "High") return "border-red-100 bg-red-50 text-red-600";
  if (value === "Medium" || value === "In Progress") return "border-[#D8B76A]/25 bg-[#FFF8E1] text-[#8A6B22]";
  if (value === "Ready" || value === "Approved") return "border-[#5B5DF5]/15 bg-[#EEF2FF] text-[#5B5DF5]";
  if (value === "Used") return "border-blue-100 bg-blue-50 text-blue-600";
...
export function AssetLibraryClient() {
  ...
  useEffect(() => {
    try {
      ...
      setAssets(stored ? JSON.parse(stored) : starterAssets);
    } catch {
  ...
  const readyAssets = assets.filter((asset) => asset.status === "Ready" || asset.status === "Approved").length;
  const progressAssets = assets.filter((asset) => asset.status === "In Progress").length;
  const projects = new Set(assets.map((asset) => asset.project).filter(Boolean)).size;
    ...
    <div className="grid gap-5 xl:grid-cols-[1.05fr_0.95fr]">
      <div className="space-y-5">
        <div className="devon-glass rounded-[2.25rem] p-6">
          <div className="mb-6 flex flex-col justify-between gap-4 md:flex-row md:items-center">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">
                Save logos, flyers, videos, files, references, and creative notes in one command library.
              </p>
            </div>
          ...
          <div className="grid gap-3 md:grid-cols-2">
            <label className="space-y-2 md:col-span-2">
              <span className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">Asset Name</span>
              <input
                value={form.name}
                onChange={(event) => setForm((current) => ({ ...current, name: event.target.value }))}
                placeholder="e.g. Official Logo, Documentary Frame 06"
            ...
            <label className="space-y-2">
              ...
              <select
                value={form.type}
                onChange={(event) => setForm((current) => ({ ...current, type: event.target.value as AssetType }))}
            ...
            <label className="space-y-2">
              <span className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">Project</span>
              <input
                value={form.project}
                onChange={(event) => setForm((current) => ({ ...current, project: event.target.value }))}
                placeholder="JRB, DevonOS, Tax Reform..."
                className="w-full rounded-2xl border border-slate-950/[0.08] bg-white/80 px-4 py-3 text-sm font-medium text-slate-800 outline-none transition placeholder:text-slate-300 focus:border-[#5B5DF5]/30 focus:bg-white focus:ring-4 focus:ring-[#5B5DF5]/10"
              />
            </label>
            ...
            <label className="space-y-2">
              <span className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">Priority</span>
              <select
                value={form.priority}
                onChange={(event) => setForm((current) => ({ ...current, priority: event.target.value as AssetPriority }))}
                className="w-full rounded-2xl border border-slate-950/[0.08] bg-white/80 px-4 py-3 text-sm font-medium text-slate-800 outline-none transition focus:border-[#5B5DF5]/30 focus:bg-white focus:ring-4 focus:ring-[#5B5DF5]/10"
              >
            ...
            <label className="space-y-2 md:col-span-2">
              <span className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">Link / File Location</span>
              <input
                value={form.link}
                onChange={(event) => setForm((current) => ({ ...current, link: event.target.value }))}
                placeholder="Google Drive link, local folder note, or URL"
                className="w-full rounded-2xl border border-slate-950/[0.08] bg-white/80 px-4 py-3 text-sm font-medium text-slate-800 outline-none transition placeholder:text-slate-300 focus:border-[#5B5DF5]/30 focus:bg-white focus:ring-4 focus:ring-[#5B5DF5]/10"
              />
            </label>
            ...
            <label className="space-y-2 md:col-span-2">
              ...
              <input
                ...
                placeholder="logo, documentary, video, official"
                className="w-full rounded-2xl border border-slate-950/[0.08] bg-white/80 px-4 py-3 text-sm font-medium text-slate-800 outline-none transition placeholder:text-slate-300 focus:border-[#5B5DF5]/30 focus:bg-white focus:ring-4 focus:ring-[#5B5DF5]/10"
              />
            </label>
            ...
            <label className="space-y-2 md:col-span-2">
              ...
              <textarea
                ...
                placeholder="Usage rules, approval reminders, design notes..."
                rows={4}
        ...
        <div className="devon-glass-dark devon-ink-shine rounded-[2.25rem] p-6 text-white">
          <p className="text-xs font-semibold uppercase tracking-[0.24em] text-white/34">Library Health</p>
          <h2 className="mt-2 text-xl font-semibold tracking-tight">Creative assets organized.</h2>
      ...
      <div className="space-y-5">
        <div className="grid gap-3 md:grid-cols-4">
          <MetricCard title="Assets" value={assets.length} sub="Saved records" icon={Package} />
          <MetricCard title="Ready" value={readyAssets} sub="Ready / approved" icon={CheckCircle2} />
          <MetricCard title="Active" value={progressAssets} sub="In progress" icon={Sparkles} />
          <MetricCard title="Projects" value={projects} sub="Asset groups" icon={Folder} />
        </div>

        <div className="devon-glass rounded-[2.25rem] p-6">
          <div className="mb-5 flex flex-col justify-between gap-4 md:flex-row md:items-center">
            <div>
              <h2 className="text-xl font-semibold tracking-tight text-[#0B0D12]">Asset Directory</h2>
              <p className="mt-2 text-sm leading-6 text-slate-500">Search, select, and manage assets.</p>
            </div>
              ...
              return (
                <div
                  ...
                  className={`rounded-[1.65rem] border p-4 shadow-[0_14px_45px_rgba(15,23,42,0.04)] transition duration-300 ${
                    isSelected ? "border-[#5B5DF5]/25 bg-[#EEF2FF]/70" : "border-slate-950/[0.08] bg-white/66 hover:bg-white"
                  }`}
                >
                  <div className="mb-4 flex justify-between gap-4">
                    <button onClick={() => setSelectedId(asset.id)} className="flex flex-1 gap-3 text-left">
                      <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-slate-950/[0.045] text-slate-700">
                        <Icon size={18} />
                      </div>

                      <div>
                        <h3 className="text-base font-semibold text-[#0B0D12]">{asset.name}</h3>
                        <p className="mt-1 text-sm font-medium text-slate-400">{asset.project || "No project"} · {asset.type}</p>
                      </div>
                    </button>
                  ...
                  <div className="mb-4 flex flex-wrap gap-2">
                    <span className={`rounded-full border px-3 py-1 text-xs font-semibold ${badgeClass(asset.status)}`}>{asset.status}</span>
                    <span className={`rounded-full border px-3 py-1 text-xs font-semibold ${badgeClass(asset.priority)}`}>{asset.priority} priority</span>
                  </div>

                  {asset.notes ? <p className="line-clamp-3 text-sm leading-6 text-slate-500">{asset.notes}</p> : null}
                </div>
              );
            })}
          </div>
        ...
        <div className="devon-glass rounded-[2.25rem] p-6">
          <h2 className="text-xl font-semibold tracking-tight text-[#0B0D12]">Asset Details</h2>
          <p className="mt-1 text-sm text-slate-400">Select an asset and copy its record.</p>

          <div className="mt-5 rounded-[1.55rem] border border-slate-950/[0.08] bg-white/70 p-4">
...
function MetricCard({
  ...
  return (
    <div className="devon-glass rounded-[1.7rem] p-5">
      ...
      <p className="mt-2 text-3xl font-semibold tracking-[-0.04em] text-[#0B0D12]">{value}</p>
